...
### 3.3 指标选择方式：实时预测 vs 回测

系统有两种指标选择模式：

| | 实时预测 (`main.py`) | 回测 (`model_predictor.py`) |
|---|---|---|
| **选择方式** | AI 大模型从 65 个中动态选 8-15 个 | 固定使用 10 个核心指标 |
| **选择依据** | 根据当天新闻、因素、市场环境智能选择 | 预设的高相关性指标子集 |
| **覆盖分类** | 每次不同，覆盖当天最相关的分类 | 固定覆盖：利率、汇率、情绪、通胀、ETF |
| **速度** | 较慢（需调用 AI，每次 ~10s） | 快（无 AI 调用） |
| **适用场景** | 实盘决策，需要最精准的预测 | 大规模历史回测，需要速度 |

**回测默认使用的 10 个核心指标**：`us_10y_yield`, `us_2y_yield`, `dxy_index`, `sp500`, `vix`, `oil_wti`, `silver`, `tips_etf`, `gld_volume`, `copper`
